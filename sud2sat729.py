	...
	for p in range(10):
		if(p > 0):
					for i in range(p+1, 10):
						sa = sa +"-"+encode((x+1),(y+1),p)+" -"+encode((x+1),(y+1),i)+" 0\n"
	return(sa);	
...
def CNFrow(x, y, value):
	...
	for t in range(1,10):
		if(t != y+1):
			sr = sr + "-"+encode((x+1),(y+1),value)+" -"+encode((x+1),t,value)+" 0\n"
...
def CNFcol(x, y, value):
	...
	for c in range(1,10):
		if(c != x+1):
			sc = sc + "-"+encode((x+1),(y+1),value)+" -"+encode(c,(y+1),value)+" 0\n"
...
def grid1(s1,f1,s2,f2,x,y,value):
	...
	for i in range(s1,f1):
		for j in range(s2,f2):
			if(i != x+1 and j != y+1):
				sg = sg + "-"+encode((x+1),(y+1),(value))+" -"+encode(i,j,value)+" 0\n"
	return sg
		...
		for y in range(9):
			...
			for z in range(10):
				if(z == 0 and puzzle[x][y][z] != 0):
					so = encode((x+1),(y+1),puzzle[x][y][z])
					so = so + " 0\n"
					count = count + 1
					break;
				elif(z == 9): 
					if(puzzle[x][y][z] == 1):
						so = so + encode((x+1),(y+1),z)
						so = so + " 0\n"
					else:
						so = so +encode((x+1),(y+1),z)
						so = so + " 0\n"
